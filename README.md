...
`tools/list` at `https://gateway.pipeworx.io/color/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/color_convert \
  -H 'Content-Type: application/json' \
  -d '{"color":"#3498db"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/color_convert`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
